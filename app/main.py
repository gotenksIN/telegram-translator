...
from app.preview import extract_preview_url, fetch_preview_text
...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if replied_message is None:
        await update.effective_message.reply_text("Please reply to a message containing Twitter URL")
        return

    preview_url = extract_preview_url(replied_message)
    if preview_url is None:
        await update.effective_message.reply_text("Could not find a LinkCleaner Twitter preview in the replied message.")
        return

    now = time.monotonic()
    semaphore = context.application.bot_data[TRANSLATION_SEMAPHORE_KEY]
    if semaphore.locked():
        await update.effective_message.reply_text("Too many translations are running. Please try again shortly.", do_quote=True)
        return
    ...
    try:
        ...
        try:
            ...
            await update.effective_message.reply_text("Could not fetch text from the replied preview message.")
            return
        ...
        try:
            ...
            await reply_long_text(update.effective_message, "Could not translate this tweet.")
            return
    finally:
        semaphore.release()

    await reply_long_text(replied_message, f"Translation:\n{translation}")
...
def main() -> None:
    ...
    application = Application.builder().token(settings.TELEGRAM_BOT_TOKEN).concurrent_updates(MAX_CONCURRENT_UPDATES).build()
